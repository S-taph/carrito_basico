...
function crearTarjetasProductosCarrito() {
  ...
  const productos = JSON.parse(localStorage.getItem("instrumentos"));
  if (productos && productos.length > 0) {
    ...
    `;
      ...
      nuevoInstrumento.getElementsByTagName("button")[0].addEventListener("click", (e) => {
          const cantidadElement = e.target.parentElement.getElementsByClassName("cantidad")[0];
          cantidadElement.innerText = restarAlCarrito(producto);
          crearTarjetasProductosCarrito();
          actualizarTotales();
        });
      nuevoInstrumento.getElementsByTagName("button")[1].addEventListener("click", (e) => {
          const cantidadElement = e.target.parentElement.getElementsByClassName("cantidad")[0];
